...
<c:forEach items="${resultList }" var="result" varStatus="status">
	<tr>
		...
		<td><input type="hidden" name="sujip_sang_name" value="${result.L }" />${result.L }</td>
		<td><input type="hidden" name="sabang_jumun_num" value="${result.I }" />${result.I }</td>
		<td><input type="hidden" name="sujip_option" value="${result.M }" />${result.M }</td>
		...
		<td><input type="hidden" name="suryang" value="${result.N }" />${result.N }</td>
		<td><input type="hidden" name="hwak_suryang" value="" /></td>
		<td><input type="hidden" name="jumun_name" value="${result.A }" />${result.A }</td>
		<td><input type="hidden" name="jumun_tel1" value="" /></td>
		<td><input type="hidden" name="jumun_tel2" value="${result.B }" />${result.B }</td>
		<td><input type="hidden" name="suchwi_name" value="${result.C }" />${result.C }</td>
		<td><input type="hidden" name="suchwi_tel1" value="${result.E }" />${result.E }</td>
		<td><input type="hidden" name="suchwi_tel2" value="${result.D }" />${result.D }</td>
		...
		<td><input type="hidden" name="taekbaesa" value="${result.O }" />${result.O }</td>
		<td><input type="hidden" name="songjang_num" value="${result.P }" />${result.P }</td>
	</tr>
</c:forEach>
